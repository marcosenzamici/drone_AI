% Crea un oggetto drone per interfacciarsi con il drone Parrot Mambo
parrotObj = parrot("Mambo");

%  Decollo drone 
takeoff(parrotObj);

% Decollo drone se parte da terra per sistemare l'altezza del drone
moveup(parrotObj,0.5,0.5);
...
function error = trackFace(parrotObj, info, w, pid, pError, fbRange)
...
% Stampa la velocità di rotazione (speed) e il movimento avanti/indietro (fb).
   ...
   % Movimento drone avanti/indietro e velocità di rotazione
    move(parrotObj,0.05,'Pitch',deg2rad(fb),'RotationSpeed',deg2rad(speed));
end

% Avvia la cattura video dalla webcam
cam = camera(parrotObj, 'FPV'); 
...
while true
    ...
    pError = trackFace(parrotObj, info, w, pid, pError, fbRange);
